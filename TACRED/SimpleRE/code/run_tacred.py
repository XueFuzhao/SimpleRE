            ...
            for i, token in enumerate(example.sentence):
                ...
                if i >= example.span1[0] and i <= example.span1[1]:
                    for sub_token in tokenizer.tokenize(token):
                        ...
                        SUBJECT_TOKENS.append(sub_token)
                if i >= example.span2[0] and i <= example.span2[1]:
                    for sub_token in tokenizer.tokenize(token):
                        OBJECT_TOKENS.append(sub_token)
            if mode == "text_ner":
                tokens = tokens + [SEP] + [SUBJECT_START] +SUBJECT_TOKENS+ [SUBJECT_END] + [CLS] + [OBJECT_START] +OBJECT_TOKENS+[OBJECT_END] +[SEP]
            else:
                tokens.append(SEP)
        else:
            subj_tokens = []
    ...
    if args.do_train:
        ...
        for lr in lrs:
            model = BertForSequenceClassification.from_pretrained(
                ...
                #model,  cache_dir=str(PYTORCH_PRETRAINED_BERT_CACHE), num_labels=num_labels, args=args)
            #if args.fp16:
            #    model.half()
            model.to(device)
            if n_gpu > 1:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature_mode", type=str, default="text_ner", choices=["text", "ner", "text_ner", "ner_text"])

...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_id,number_of_relations):
...
class bertdProcessor(DataProcessor):  # bert
    def __init__(self):
        random.seed(42)
        self.D = [[], [], []]
        for sid in range(3):
    ...
    def _create_examples(self, data, set_type):
        ...
        for (i, d) in enumerate(data):
            ...
            text_c = []
            label = []
            for tu in range(len(data[i])-1):

                text_b.append(tokenization.convert_to_unicode(data[i][1+tu][0]))

                text_c.append(tokenization.convert_to_unicode(data[i][1 + tu][1]))
                label.append(data[i][1 + tu][2])
...
class bertdf1cProcessor(DataProcessor):  # bert (conversational f1)
    def __init__(self):
        random.seed(42)
        self.D = [[], [], []]
        for sid in range(1,3):
...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for k in range(len(example.text_b)):

            tokens_b += tokenize(example.text_b[k], tokenizer)

            tokens_c += tokenize(example.text_c[k], tokenizer)

        _truncate_seq_tuple(tokens_a, tokens_b, tokens_c, max_seq_length - 2 - int(len(example.text_b)*2))
        ...
        for k in range(len(example.text_b)):
            ...
            tokens_b += tokenize(example.text_c[k], tokenizer)
            tokens_b += ["[SEP]"]

        number_of_relations = len(example.text_b)
        ...
        if ex_index < 5:
            ...
            logger.info("number_of_relations: %s" % (number_of_relations))
...
def f1_eval(logits, features):
    ...
    for f in features:
        ...
        for m in range(len(f[0].label_id)):
            label = []
            for i in range(36):
                if f[0].label_id[m][i] == 1:
                    label += [i]
            if len(label) == 0 and  f[0].label_id[m][0]!= -1:
                label = [36]
            if len(label) != 0:
                labels += [label]
...
def main():
    ...
    processors = {
        ...
        "bertd": bertdProcessor,
        "bertdf1c": bertdf1cProcessor,
    }
    ...
    if args.do_eval:
        ...
        for f in eval_features:
            label_seq = f[0].label_id
            if len(f[0].label_id) < 40:
                while(len(label_seq) < 40):
                    label_seq.append(padding_label)
            label_id.append(label_seq)
            num_of_relations.append(f[0].number_of_relations)

        all_input_ids = torch.tensor(input_ids, dtype=torch.long)
        all_input_mask = torch.tensor(input_mask, dtype=torch.long)
        all_segment_ids = torch.tensor(segment_ids, dtype=torch.long)
        all_label_ids = torch.tensor(label_id, dtype=torch.float)
        all_num_of_relations = torch.tensor(num_of_relations, dtype=torch.long)

        eval_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids,all_num_of_relations)
        if args.local_rank == -1:
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_num_of_relations )
        if args.local_rank == -1:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, label_ids,num_of_relations = batch
                loss, _ , true_label = model(input_ids, segment_ids, input_mask, labels = label_ids,num_of_relations = num_of_relations, n_class=1)
                if n_gpu > 1:
                    loss = loss.mean()
            ...
            logits_all = []
            for input_ids, input_mask, segment_ids, label_ids, num_of_relations in eval_dataloader:
                ...
                with torch.no_grad():
                    tmp_eval_loss, logits, true_label = model(input_ids, segment_ids, input_mask, label_ids, num_of_relations,1)
                ...
                tmp_eval_accuracy = accuracy(logits, true_label.reshape(-1))
